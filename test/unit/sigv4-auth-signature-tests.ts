...
import { testingOnly, computeSigV4SignatureCassandraRequest } from '../../lib/sigv4-auth-signature'

describe('Signing Steps', () => {
  const signingSteps = testingOnly.signingSteps
  ...
  const signingKeyHex = '7fb139473f153aec1b05747b0cd5cd77a1186d22ae895a3a0128e699d72e1aba'
  const signingKey = Buffer.from(signingKeyHex, 'hex')
  const computedFullResult =
  ...
  describe('#deriveCannonicalRequest()', () => {
    it('should work with example above', () => {
      ...
      let actual = signingSteps.deriveCanonicalRequest(accessKeyId, signingScope, isoDateStamp, nonceHash)
  ...
  describe('#createStringToSign()', () => {
    it('should format everything correctly', () => {
      let actual = signingSteps.createStringToSign(canonicalRequest, isoDateStamp, signingScope)
  ...
  describe('#deriveSigningKey()', () => {
    it('should produce key when given valid input', () => {
      const actual: Buffer = signingSteps.deriveSigningKey(secretAccessKey, credentialTimeStamp, region)

      assert.equal(actual.toString('hex'), signingKeyHex)
    })
  })
  ...
  describe('#computeSigV4SignatureCassandraRequest()', () => {
    it('should produce valid result when no temporary creds', () => {
      let actual = computeSigV4SignatureCassandraRequest({
        region: region,
        nonce: nonce,
        date: requestDate,
    ...
    it('should produce valid result when given temporary creds', () => {
      ...
      const computedFullResultWithSession =
        'signature=7f3691c18a81b8ce7457699effbfae5b09b4e0714ab38c1292dbdf082c9ddd87' +
        ',access_key=UserID-1,amzdate=2020-06-09T22:41:51.000Z' +
        ',session_token=sess-token-1'

      const actual = computeSigV4SignatureCassandraRequest({
        region: region,
        nonce: nonce,
        date: requestDate,
        accessKeyId: accessKeyId,
